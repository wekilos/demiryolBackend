...
const create = (req, res) => {
    const { title_tm, title_en, title_ru, text_tm, text_en, text_ru } =
        req.body;

    let randomNumber_tm = Math.floor(Math.random() * 999999999999);
    let img_direction_tm =
        `./uploads/` + randomNumber_tm + `${req.files.picture.name}`;
    fs.writeFile(img_direction_tm, req.files.picture.data, function (err) {
        console.log(err);
    });
...
const update = (req, res) => {
    const { title_tm, title_en, title_ru, text_tm, text_en, text_ru, id } =
        req.body;
...
const updateFile = async (req, res) => {
    ...
    if (data) {
        fs.unlink(data.picture, function (err) {
            if (err) throw err;
        });
